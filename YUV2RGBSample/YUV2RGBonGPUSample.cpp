...
// const char* fragmentShaderSource = R"(
// precision highp float; // 影片處理強烈建議使用高精度
// varying vec2 vTexCoord;
...
// // 分段線性插值 (含原點 0,0 錨定邏輯)
// float interpolate(float inputVal, float y0, float y1, float y2, float y3, float y4) {
//     // 為了效能，這些 uniform 讀取通常編譯器會優化，但若能寫死 const float 會更快
...
//     // 線性插值公式
//     return clamp(y_low + (y_high - y_low) * (inputVal - x_low) / denominator, 0.0, 1.0);
// }

// void main() {
//     // 1. 【關鍵步驟】將 YUV 轉為 RGB
//     // 如果沒有這步，後面的 inputColor 就不正確，補償也會錯亂
//     vec3 inputColor = yuv2rgb(vTexCoord);

//     // 2. 採樣 5 個控制點 (並做反 Gamma 修正: sqrt)
//     // 假設控制點 BMP 是 Gamma 2.2 儲存的，而我們需要在線性空間運算
    
//     // R Channel Controls
//     float r_cp0 = sqrt(texture2D(uControlPoint0, vTexCoord).r);
//     float r_cp1 = sqrt(texture2D(uControlPoint1, vTexCoord).r);
//     float r_cp2 = sqrt(texture2D(uControlPoint2, vTexCoord).r);
//     float r_cp3 = sqrt(texture2D(uControlPoint3, vTexCoord).r);
//     float r_cp4 = sqrt(texture2D(uControlPoint4, vTexCoord).r);

//     // G Channel Controls
//     float g_cp0 = sqrt(texture2D(uControlPoint0, vTexCoord).g);
//     float g_cp1 = sqrt(texture2D(uControlPoint1, vTexCoord).g);
//     float g_cp2 = sqrt(texture2D(uControlPoint2, vTexCoord).g);
//     float g_cp3 = sqrt(texture2D(uControlPoint3, vTexCoord).g);
//     float g_cp4 = sqrt(texture2D(uControlPoint4, vTexCoord).g);

//     // B Channel Controls
//     float b_cp0 = sqrt(texture2D(uControlPoint0, vTexCoord).b);
//     float b_cp1 = sqrt(texture2D(uControlPoint1, vTexCoord).b);
//     float b_cp2 = sqrt(texture2D(uControlPoint2, vTexCoord).b);
//     float b_cp3 = sqrt(texture2D(uControlPoint3, vTexCoord).b);
//     float b_cp4 = sqrt(texture2D(uControlPoint4, vTexCoord).b);

//     // 3. 執行插值補償
//     // 使用 inputColor 的 RGB 值去查表 (Map) 得到新的 RGB 值
//     float outR = interpolate(inputColor.r, r_cp0, r_cp1, r_cp2, r_cp3, r_cp4);
//     float outG = interpolate(inputColor.g, g_cp0, g_cp1, g_cp2, g_cp3, g_cp4);
//     float outB = interpolate(inputColor.b, b_cp0, b_cp1, b_cp2, b_cp3, b_cp4);
